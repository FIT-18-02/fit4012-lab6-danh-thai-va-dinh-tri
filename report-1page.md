...
## Mục tiêu
Phạm Danh Thái : Bài lab nhằm xây dựng hệ thống gửi và nhận dữ liệu qua socket TCP với mã hóa AES-CBC. Hệ thống tách biệt hai kênh truyền: **key channel** để gửi AES key và IV, **data channel** để gửi ciphertext. Sinh viên hiểu được luồng hoạt động của mã hóa đối xứng trong ứng dụng mạng, cách sử dụng PKCS#7 padding, length header, và phân tích các điểm yếu bảo mật khi key được gửi ở dạng plaintext.

## Phân công thực hiện

| Vai trò | Phụ trách chính |
|---------|----------------|
| **Sender** (mã hóa, key channel, data channel) | Nguyễn Đình Trí |
| **Receiver** (giải mã, nhận packet, xử lý lỗi) | Phạm Danh Thái |
| **Test cases** (pytest, wrong key, tamper) | Cả hai |
| **Log & Minh chứng** | Nguyễn Đình Trí |
| **Threat model** | Phạm Danh Thái |
| **Báo cáo & Phần chung** | Cả hai |

## Cách làm


### 1. Sender (Phạm Danh Thái thực hiện)
...
### 2. Receiver (Nguyễn Đình Trí thực hiện)
...
Phạm Danh Thái : Tóm tắt kết quả chạy, log minh chứng, output nhận được và các test quan trọng.
...
Nguyễn Đình Trí : Rút ra bài học kỹ thuật và bài học bảo mật từ bài lab.
